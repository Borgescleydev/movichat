...
## F-216 | categoria: funcional | severidade: alta | status: aberto
...
- Observado: como nada chama o endpoint num schedule, os dispatches ficam `pending` para sempre e a campanha presa em `scheduled`/`running`. **Verificação operacional necessária (fora do código):** confirmar se existe um job ativo em cron-job.org (ou equivalente) apontando para a URL de produção. Se não existir / estiver desativado / a URL de produção mudou → este é o motivo. O fix de código seria (a) adicionar `crons` em `vercel.json`/`vercel.ts` se o plano permitir o intervalo desejado, ou (b) garantir o cron externo. NÃO alterado (read-only).

## F-217 | categoria: funcional | severidade: media | status: aberto
- Arquivo: `app/api/cron/status/route.ts:44-49`.
- Causa: a resposta do endpoint de status declara `schedule: "* * * * *"`, `description: "Executa a cada minuto (Vercel Cron Jobs)"` — **contradiz** o mecanismo real (cron externo) descrito na UI (`CronSettings.tsx:212,218`) e a ausência de `crons` no `vercel.json` (F-216). Essa string é puramente cosmética: NÃO cria nenhum agendamento; o Vercel só roda cron se houver `crons` no `vercel.json`/`vercel.ts`, o que não existe.
- Esperado: o painel de status deve refletir que o gatilho depende de um cron externo (ou de `crons` configurado), para o operador não assumir que "o Vercel já cuida disso".
- Observado: o texto "Vercel Cron Jobs" induz o operador a achar que o agendamento é automático, mascarando a causa real (F-216). Contribui diretamente para o sintoma por desorientar o diagnóstico. NÃO alterado.
...
## F-217 | categoria: funcional | severidade: alta | status: aberto
...
- (resolvedor) Correção:
  ...
  - **Validação:** `npx tsc --noEmit` sem erros; `eslint` nos dois arquivos sem erros (só warnings pré-existentes). Relacionado a F-313 (mesmo handler/sintoma de "Erro ao remover", causa diferente — F-313 tratou órfã stale-connected via check de status ao vivo; este trata o 500 opaco da FK de `QuickDispatch` + a troca de bloqueio por confirmação).
