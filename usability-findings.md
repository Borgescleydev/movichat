...
- (resolvedor) Correção:
  ...
  - **Validação:** `npx tsc --noEmit` sem erros; `eslint` no arquivo tocado sem erros (só warnings pré-existentes). Relacionado a F-313 (mesmo handler/sintoma de "Erro ao remover", causa diferente — F-313 tratou órfã stale-connected via check de status ao vivo; este trata o 500 opaco da FK de `QuickDispatch` + a troca de bloqueio por confirmação).
