...
const ALL_NAV: Array<{
  href: string;
  label: string;
  permKey: keyof UserPerms | null;
  superadminOnly?: boolean;
  optIn?: boolean; // requires explicit true; defaults to hidden for agents
  icon: React.ReactNode;
}> = [
  {
    href: "/dashboard", label: "Dashboard", permKey: "reports" as keyof UserPerms,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>),
  },
  {
    href: "/pipeline", label: "Pipeline", permKey: "pipeline" as keyof UserPerms,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2" /></svg>),
  },
  {
    href: "/contacts", label: "Contatos", permKey: "contacts" as keyof UserPerms,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>),
  },
  {
    href: "/conversations", label: "Conversas", permKey: "conversations" as keyof UserPerms,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>),
  },
  {
    href: "/campaigns", label: "Campanhas", permKey: "campaigns" as keyof UserPerms,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" /></svg>),
  },
  {
    href: "/individual", label: "Disparo Individual", permKey: "individual" as keyof UserPerms, optIn: true,
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13l2 2-2 2M16 15h6" /></svg>),
  },
  {
    href: "/settings", label: "Configurações", permKey: null, // always visible
  ...
  {
    href: "/changelog", label: "Versionamento", permKey: null, superadminOnly: true,
    ...
    >
      ...
      <nav className="flex-1 p-4 space-y-1">
        {ALL_NAV.filter(item => {
          ...
          if (item.optIn) return user.permissions?.[item.permKey] === true;
          return user.permissions?.[item.permKey] !== false;
        }).map((item) => {
